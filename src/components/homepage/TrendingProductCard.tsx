...
const TrendingProductCard = ({ fish }: { fish: TProduct }) => {
  return (
    <div>
      <div className="flex flex-col max-w-lg p-6 space-y-6 overflow-hidden rounded-lg shadow-md dark:bg-gray-50 dark:text-gray-800">
        <div>
          <Image
            ...
            src={fish?.imageURL}
            alt="image"
            className="mb-4 mx-auto h-[200px] sm:h-[300px] dark:bg-gray-500 rounded-md"
          />
          <h2 className="mb-1 text-xl font-semibold">
            {fish?.title} {fish?.category}
          </h2>
          <p className="text-sm dark:text-gray-600 inline">
        ...
        <div className="flex flex-wrap justify-between">
          ...
          <div className="flex space-x-2 text-sm dark:text-gray-600">
            <button type="button" className="flex items-center p-1 space-x-1.5">
              ...
              <span>{Math.floor(Math.random() * (30 - 10 + 1)) + 10}</span>
            </button>
            <button type="button" className="flex items-center p-1 space-x-1.5">
              <svg
                xmlns="http://www.w3.org/2000/svg"
